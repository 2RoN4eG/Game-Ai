#include "t_ramping_up_vehicle_speed_subsystem.hpp"
...
namespace
{
    ...
    constexpr t_frame_delta_time_value g_rumping_up_time { 10000. };    // milliseconds
...
t_ramping_up_vehicle_speed_subsystem::t_ramping_up_vehicle_speed_subsystem(t_vehicle_component& vehicle,
...
void t_ramping_up_vehicle_speed_subsystem::update(const t_frame_delta_time_value delta_time)
{
    ...
    const t_speed_value speed_scale { speed /  _speed_limit };

    if (_speed_scale_range.is_maximum(speed_scale))
    {
        return;
    }

    const t_delta_speed_value delta_speed { delta_time / g_rumping_up_time * 100 };
